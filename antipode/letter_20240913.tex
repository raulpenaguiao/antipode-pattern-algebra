...
We appreciate the effort and comments demonstrated by the referees to carefully review our paper.
We have taken this information to heart and overhauled the paper.

Main changes are that we rewrote from scratch the introduction chapter to reflect how all pieces fit together, as well as we included a new chapter that displays an application of this antipode formula. 

On rewriting the introduction, we were careful in referencing that pattern Hopf algebras arise from any species with restrictions, pushed Figure 1 to chapter 3, and added helpful pointers to specific elements of the article as suggested.

When we submitted this paper we were under the impression that we would be leaving a concrete application of the antipode formula for another publication, but ostensibly that did not quite come to fruition.
...
Specifically, the notion of "$\mathrm{eval}^{\tau}$ and "$n$ covers of $\pi$" have been pushed to this last chapter.

We also rewrote the parking function part on the abstract to reflect the content.
We want to point out that it is not our goal in this paper to further explore the antipode formula on parking functions, merely present a construction of parking functions that is amenable to species with restrictions.
This is why we do not find a chapter about parking functions on section 7.
